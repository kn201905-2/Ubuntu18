...
以上で code-server は利用可能となるが、今後、code-server を別バージョンに差し替えても実行パスを変えなくて済むように、シンボリックリンクを張っておく  
> ln -s ./code-server1.1156-vsc1.33.1-linux-x64/code-server .  

* フォントの変更  
> 「Ctrl + ,」で「font family」を検索して、「Editor:Font Family」に「Consolas」を指定する  
> 「Ctrl + ,」で「font size」を検索して、「Editor:Font Size」に「12」を指定する  
> 「Ctrl + ,」で「Line Height」を検索して、「Editor:Line Height」に「20」を指定する  

* 種々の設定「Ctrl+Shift+p」で、setting と入力し「Open Settings（JSON）」を選択  
